...
Function main
 ...
 ' use your robot actual IP address or 127.0.0.1 for simulator at local host
  ...
  Do
   ...
   ' if the command is move x y z
   If LCase$(indata$(0)) = "move" Then
     	x = Val(Trim$(indata$(1)))
    	y = Val(Trim$(indata$(2)))
	    z = Val(indata$(3))
   ...
   ' if the command is pick x y v
   If LCase$(indata$(0)) = "pick" Then
		x = Val(Trim$(indata$(1)))
    	y = Val(Trim$(indata$(2)))
	    v = Val(indata$(3))
		
		Print "Picking from x=", x, " y=", y, " z=", z, " u=", u, " v=", v
		Jump3 Here +Z(50), Here :X(x) :Y(y) :Z(z + 50) :V(v), Here :X(x) :Y(y) :Z(z) :V(v)
